...
def make_train_dsmec_env(all_args, mec_config):
    env = EnvironmentManager(mec_config)
    return env


def make_eval_dsmec_env(all_args, mec_config):
    env = EnvironmentManager(mec_config)
    return env
...
def main(args):
    ...
    envs = make_train_dsmec_env(all_args, mec_config)
    eval_envs = make_eval_dsmec_env(all_args, mec_config) if all_args.use_eval else None
    num_agents = mec_config.es_cs_set_config.user_device_num  # all_args.num_agents
    ...
    if all_args.share_policy:
        if all_args.env_name == "GraphMPE" or all_args.env_name == "DSMEC":
