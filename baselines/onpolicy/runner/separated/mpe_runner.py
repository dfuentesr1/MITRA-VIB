...
class MPERunner(Runner):
    ...
    def run(self):
        ...
        for episode in range(int(self.mec_config.train_config.episode_num)):
            ...
            for step in range(self.episode_length):
                ...
                state_class_list = []
                obs = []
                num_agent = len(env.es_cs_set.all_user_device_list)
                ...
                scaled_a_list = []
                action_class_list = []
                for i in range(len(env.es_cs_set.all_user_device_list)):
                    a = acts_batch[i]
                    ...
                    meaningful_scaled_a[0] = int(
                        np.floor((scaled_a[0] + 1) * env.es_cs_set.es_cs_num / 2))
            ...
            env.writer.add_scalar("data/avg_cost", episode_cost_list[-1], episode)
            env.writer.add_scalar("data/avg_cost_max", episode_cost_max_list[-1], episode)
            env.writer.add_scalar("data/0timesteps_in_episode", episode_timesteps, episode)
            env.writer.add_scalar("data/success_rate",
    ...
    def warmup(self):
        ...
        state_class_list = []
        obs = []
        num_agent = len(self.envs.es_cs_set.all_user_device_list)
        for user_device_id in range(len(self.envs.es_cs_set.all_user_device_list)):
            each_state = self.envs.get_state_per_user_device(user_device_id)
            state_class_list.append(each_state)
            state_list = each_state.get_state_list()
            state_array = each_state.get_normalized_state_array()
            obs.append(state_array)
