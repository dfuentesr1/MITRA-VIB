...
class VIBEncoder(nn.Module):
    def __init__(self, state_dim, z_dim=64, width=128):
        super().__init__()
        self.backbone = nn.Sequential(
            nn.Linear(state_dim, width), nn.SiLU(),
            nn.Linear(width, width), nn.SiLU()
        )
        self.mu     = nn.Linear(width, z_dim)
        self.logvar = nn.Linear(width, z_dim)  # 输出对数方差
    def forward(self, s):
        h = self.backbone(s)
        mu = self.mu(h)
        logvar = torch.tanh(self.logvar(h)) * 4.0  # 稍作约束，避免数值爆炸
        std = (0.5*logvar).exp()
...
class MA_T_DDPG(object):
    def __init__(self, num_agent, obs_dim, state_dim, action_dim_list, max_action, deivce ,max_mi_c,min_mi_c):
      ...
      #  self.pr_actor_optimizer = torch.optim.Adam([{'params': self.pr_actor.parameters()}],lr=3e-5)
        ...
        self.z_dim = 64
        self.encoder = VIBEncoder(state_dim, self.z_dim, 128).to(self.device)
        self.encoder_optimizer = torch.optim.Adam(self.encoder.parameters(), lr=1e-3)

        # !!! 关键改动：把 state_dim -> z_dim
        self.mine_policy = NEW_MINE(self.z_dim, self.total_action).to(self.device)
        self.mine_optimizer = torch.optim.Adam([{'params': self.mine_policy.parameters()}], lr=0.0001)

        self.club_policy = CLUB(self.z_dim, self.total_action, 64).to(self.device)
    ...
    def train_mine(self, replay_buffer, iterations, batch_size=64):
        ...
        for it in range(iterations):
            ...
            mu, logvar,z = self.encoder(pos_state)               
            pos_loaa, pos_MI = self.mine_policy(z, torch.cat(tp_pos_action, -1))

            loss = pos_loaa.mean() + 0.001 * (0.5 * torch.sum(torch.exp(logvar) + mu**2 - 1.0 - logvar, dim=1).mean())
    ...
    def train_club(self,replay_buffer,iterations,batch_size=64):
        ...
        for it in range(iterations):
            ...
            pos_state = torch.FloatTensor(pos_state).to(self.device)
            mu, logvar, z = self.encoder(pos_state)
            club_loss = self.club_policy.learning_loss(z, copy_action) + 0.001 * (0.5 * torch.sum(torch.exp(logvar) + mu**2 - 1.0 - logvar, dim=1).mean())

            min_MI_loss_list.append(club_loss.cpu().data.numpy())
            self.club_optimizer.zero_grad()
            self.encoder_optimizer.zero_grad()  
            club_loss.backward()
            torch.nn.utils.clip_grad_norm_(self.encoder.parameters(), 0.9)  
    ...
    def train_actor_with_mine(self, replay_buffer, iterations, batch_size=64, discount=0.99, tau=0.001, max_mi_c=0.0,
        ...
        for it in range(iterations):
            ...
            _,_,z_for_adv = self.encoder(state)
            z_for_adv = z_for_adv.detach()
            #### TODO update Q
            next_action_list = []
            ...
            if min_adv_c > 0.0 :
                with torch.no_grad():
                    MI_upper_bound = self.club_policy(z_for_adv, action).detach()
                MI_upper_bound = MI_upper_bound.reshape(reward.shape)
            else :
                MI_upper_bound = 0.0*torch.zeros_like(reward)
            if max_adv_c > 0.0 :
                with torch.no_grad():
                    neg_MI, half_MI= self.mine_policy(z_for_adv, action)
            ...
            training_r_MITRAVIB_list.append(torch.mean(- min_adv_c*MI_upper_bound + max_adv_c * MI_lower_bound, dim=0).item())
            ...
            mi_sum_loss = 0.0
            _,_,z_detached = self.encoder(state)
            z_detached = z_detached.detach()
            if min_mi_c > 0.0 :
                min_upper_bound= self.club_policy(z_detached, torch.cat(gen_old_action, 1))
                ...
                min_mi_loss = 0.0
            if max_mi_c > 0.0:
                neg_max_lower_bound,_ = self.mine_policy(z_detached, torch.cat(gen_old_action, 1))
        ...
        return np.mean(Q_loss_list),np.mean(min_mi_list),np.mean(max_mi_list),0.0,0.0, np.mean(MI_upper_bound_list), np.mean(MI_lower_bound_list), np.mean(training_reward_list), np.mean(training_reward_Q_list), np.mean(training_r_MITRAVIB_list)
