...
class EnvironmentManager:
    def __init__(self, global_config: GlobalConfig):
        ...
        self.task_config = self.es_cs_set_config.task_config
        ...
        self.es_cs_set = StationSet(self.global_config)
        self.cost_array_all_available_step_in_episode = np.zeros(
            (self.global_config.train_config.step_num, self.es_cs_set.user_device_num)) 
        self.reward_array_all_available_step_in_episode = np.zeros(
            (self.global_config.train_config.step_num, self.es_cs_set.user_device_num))

    def reset(self):
        self.es_cs_set = StationSet(self.global_config)
        # self.next_es_cs_set = copy.deepcopy(self.es_cs_set)

        self.es_cs_set.shuffle_task_size_list()
        self.es_cs_set.update_all_user_device_message()
        self.cost_array_all_available_step_in_episode = np.zeros(
            (self.global_config.train_config.step_num, self.es_cs_set.user_device_num))
        self.reward_array_all_available_step_in_episode = np.zeros(
            (self.global_config.train_config.step_num, self.es_cs_set.user_device_num))
    ...
    def create_task_per_step(self):
        self.es_cs_set.update_all_user_device_message()
        for es_cs in self.es_cs_set.es_cs_list:
            es_cs.priority_task_list.clear()
    ...
    def step(self, state_class_list, action_class_list, step_count):
        ...
        for idx, each_user_device_state in enumerate(state_class_list):
            cur_user_device_offload_choice_idx = action_class_list[idx].offload_choice_idx
            if cur_user_device_offload_choice_idx < 0 or cur_user_device_offload_choice_idx == self.es_cs_set.es_cs_num:
                print("cur_user_device_offload_choice_idx:", cur_user_device_offload_choice_idx)
                print("error!")
                exit()
            transmit_time = each_user_device_state.transmitting_time_to_all_es_cs_list[
                cur_user_device_offload_choice_idx]
            task = self.es_cs_set.all_user_device_list[idx].task
            assert task.task_from_user_device_id == idx
            self.es_cs_set.es_cs_list[cur_user_device_offload_choice_idx].priority_task_list.append(
                {'transmit_time': transmit_time, 'task': task})  # BS receive MDs' task
            # print("res:", self.es_cs_set.es_cs_list[cur_user_device_offload_choice_idx].priority_task_list)
            self.es_cs_set.all_user_device_list[
                idx].last_es_cs_offload_choice = cur_user_device_offload_choice_idx

        for idx, es_cs in enumerate(self.es_cs_set.es_cs_list):
            es_cs.priority_task_list.sort(key=lambda x: x['transmit_time'])  # each BS sort by the priority_task_list
            for task_info in es_cs.priority_task_list:
                task = task_info['task']
                es_cs.task_queue.shared_task_execute_queue.append(
                    task)
                offload_task_percentage = action_class_list[
                    task.task_from_user_device_id].offload_task_percentage

                if 1 - offload_task_percentage > 0:  # local
                    local_time = task.task_data_size * (1 - offload_task_percentage) / \
                                 self.es_cs_set.all_user_device_list[
                                     task.task_from_user_device_id].computing_ability_now
                    task.task_local_finish_time = local_time
                if offload_task_percentage > 0:  # edge
                    task_switch_time = 0
                    if not es_cs.task_queue.cur_task_s_user_device_appeared_in_queue(task):
                        task_switch_time = \
                            self.task_config.task_switch_time_matrix_on_es_cs[task.task_from_user_device_id][
                                es_cs.es_cs_id]
                        assert task_switch_time == task.task_switch_time_list_on_es_cs[
                            es_cs.es_cs_id]
                    task.task_offload_finish_time += task_info['transmit_time']
                    task_exe_time = task.task_data_size * offload_task_percentage / es_cs.computing_ability_now
                ...
                cost_array[task.task_from_user_device_id] = cost
                ...
                reward_array[task.task_from_user_device_id] = reward
                done_list[task.task_from_user_device_id] = done
                # print("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~")

            es_cs.task_queue.update_task_sum_process_time(self.es_cs_set_config.time_step_max)
        ...
        for user_device_id in range(len(self.es_cs_set.all_user_device_list)):
            each_state = self.get_state_per_user_device(user_device_id)
    ...
    def get_state_per_user_device(self, user_device_id):
        user_device = self.es_cs_set.all_user_device_list[user_device_id]
        state = State(user_device, self.es_cs_set)
        return state
