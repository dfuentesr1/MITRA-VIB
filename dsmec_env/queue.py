...
class TaskQueue:
    ...
    def cur_task_s_user_device_appeared_in_queue(self, task):
        if task.task_from_user_device_id not in self.seen_user_device_id:
            self.seen_user_device_id.append(task.task_from_user_device_id)
            return False
        else:
            return True
